...
export default function GhostBlogModal({
  isOpen,
  onClose,
  post,
  onNavigate,
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {/* Content */}
            <div
              className="flex-1 overflow-y-auto scroll-smooth p-4 md:p-5"
              style={{
                scrollbarWidth: "thin",
                scrollbarColor:
                  "color-mix(in srgb, var(--bg-color) 70%, white 8%) var(--bg-color)",
              }}
            >
              <article className="mx-auto max-w-4xl">
                ...
                {/* Read on Ghost Blog Link */}
                <div className="mt-8 text-center">
                  <a
                    href={`https://blog.nodoserrano.org/${post.slug}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-slate-400 hover:text-cyan-400 transition-colors"
                  >
                    Ver artículo original en blog.nodoserrano.org →
                  </a>
                </div>
              </article>
            </div>
